...
DWORD testScanner(HWND genshinWnd, tesseract::TessBaseAPI* api, std::function<void(artifact::IArtifact&)> proc, bool getArtifacts) {
    RECT rect;
    ...
    for (size_t i = 0; i < 5; i++) {
        for (size_t j = 0; j < 8; j++) {
            ...
            artifact::IArtifact artifactData;
            if (getArtifacts)
                artifactData = artifact::getArtifactData(api, screenPix);
            proc(artifactData);
