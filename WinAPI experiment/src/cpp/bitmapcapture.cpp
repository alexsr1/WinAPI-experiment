...
namespace bmp {
    ...
    BOOL CALLBACK enumWindowCallback(HWND hWnd, LPARAM lParam) {
        ...
        std::string title;
        title.resize(maxLength, ' ');

        if (!IsWindowVisible(hWnd)) {
            return TRUE;
        }

        UINT charsCopied = GetWindowTextA(hWnd, title.data(), maxLength);
        DWORD pId = 0;
    ...
    void captureScreen(const char path[])
    {
        ...
        DWORD colorTableSize = (1 << bi.biBitCount);

        // Offset to where the actual bitmap bits start.
        bmfHeader.bfOffBits = (DWORD)sizeof(BITMAPFILEHEADER) + (DWORD)sizeof(BITMAPINFOHEADER) + colorTableSize;
        ...
        colorTable[0] = { 0,0,0,0 };
        colorTable[1] = { 255,255,255,0 };

        WriteFile(hFile, (LPSTR)&bmfHeader, sizeof(BITMAPFILEHEADER), &dwBytesWritten, NULL);
        WriteFile(hFile, (LPSTR)&bi, sizeof(BITMAPINFOHEADER), &dwBytesWritten, NULL);
        WriteFile(hFile, (LPSTR)&colorTable, colorTableSize, &dwBytesWritten, NULL);
